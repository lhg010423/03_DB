...
-- 테이블에 값만 복사되는 거지 기능같은거는 복사 안됨 DEFAULT값 복사X
...
-- 1) INSER INTO 테이블명 VALUES(데이터, 데이터, 데이터,....)
...
ROLLBACK; -- 장채현 만든 행이 사라짐, DB에 옮기기전 바구니에있는 INSERT구문이 엎어짐
...
COMMIT; -- DB에 보내기전에 있는 바구니(트랜잭션) 안에 있는 INSERT가 DB로 전송됨 실제로 값이 바뀜
...
CREATE TABLE EMP_01(
		...
		DEPT_TITLE VARCHAR2(20) -- 가변길이에 관한 거(문자열 길이)
);
...
-- 서브붜리(SELECT) 결과를 EMP_01 테이블에 INSERT
