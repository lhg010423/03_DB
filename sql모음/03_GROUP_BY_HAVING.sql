 ...
 * 6 : ORDER BY 컬럼명 | 별칭 | 컴럼순번 정렬방식(ASC/DESC) [NUILL FIRST | LAST
 * 
 */
...
-- 그룹으로 묶는 값에 대해서 SELECT절에서 그룹함수를 사용함

-- 그룹함수는 단 한개의 결과값만 산출하기 떄문에 그룹이 여러개일 경우 오류 발생
-- 여러 개의 결과값을 산출하기 위해 그룹함수가 적용된 그룹의 기준을 ORDER BY절에 기술하여 작성
...
-- 2) 전체 급여 합 조회
SELECT SUM(SALARIY) FORM EPLOYEE; -- 1행
...
SELECT DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여' ) 성별,
	...
	SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여' ) -- 별칭사용X(SELECT 절 해석X)
...
-- 그룹함수가 적용되지 않은 컬럼을
-- 모두 GROUP BY절을 작성해야 함


-- EMPLOYEE 테이블에서 부서별로 같은 직급인 사원의 수를 조회
-- 부서코드 오름차순, 직급코드 내림차순

SELECT DEPT_CODE, JOB_CODE, COUNT(*) -- COUNT가 그룹화한거
FROM EMPLOYEE
