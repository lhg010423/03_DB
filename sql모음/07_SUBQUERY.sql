 ...
 * 
 * - 다중열 서브쿼리 : 서브쿼리의 SELECT 절에 나열된 항복수가 여러개일 때
 * 
 * - 다중행 다중열 서브쿼리 : 조회 결과 행 수와 열 수가 여러개일 때
 * 
 * - 상(호연)관 서브쿼리 : 서브쿼리가 만들 결과 값을 메인쿼리가 비교 연산할 때
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID) -- 직급코드가 없는 직원도 있어서 LEFT를 씀
...
-- 2) 부서별 급여합이 17700000인 부서의 부서명과 급여함 조회

SELECT DEPT_TITLE, SUM(SALARY)
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
GROUP BY DEPT_TITLE
HAVING SUM(SALARY) = 017700000;
...
-- * SELECT 절에도 서브뭐리 사용할 수 있음

-- 선택함수 사용
--> DECODE(컬럼명, 값1, 1인경우, 값2, 2인경우,.. 일치하지 않은경우)
...
-- * 집합 연산자(UNION, 합집합) 사용방법, 3번 4번 복붙
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, '사수' 구분
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- 1) LOCALTION 테이블에서 NATIONAL_CODE가  KO인 경우의 LOCAL_CODE 조회
SELECT LOCAL_CODE
FROM LOCATION
...
-- 1) 퇴사한 여직원 조회하기
SELECT DEPT_CODE, JOB_CODE -- 다중열 서브쿼리
FROM EMPLOYEE
WHERE ENT_YN = 'Y'
...
/* IN 서브쿼리 메인쿼리의 조건을 만족하는 데이터를 반환함
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE) -- 부서없는 직원도 표시해야하니 LEFT를 씀
WHERE ENT_YN = 'N';
/*
...
--		SQL에서 단일 값을 가르켜 '스칼라' 라고 함, 그냥 값이 하나
--> SELECT절에 작성되는 단일행 서브쿼리

-- 모든 직원의 이름, 직급, 급여, 전체 사원 중 가장 높은 급여와 차
SELECT EMP_NAME, JOB_CODE, SALARY,
...
-- 왜? 부서별로 가장 빠른입사자 구했을 때 D8 부서는 이태림임(이태림 퇴사자)
...
TO_CHAR(SALARY * ( 1 + NVL(BONUS, 0) ) * 12, 'L999,999,999') "보너스 포함 연봉"
FROM EMPLOYEE MAIN
JOIN JOB ON JOB.JOB_CODE = MAIN.JOB_CODE -- CHAT-GPT 가 만든 JOIN
--JOIN JOB USING(JOB_CODE) <-- 오류남
