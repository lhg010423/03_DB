...
-- 함수는 SELECT문의
-- SELECT 절, WHERE 절, ORDER BY절, GROUP BY절, HAVING절 사용가능
...
SELEFT ROUND(123.456, -1) FROM DUAL; -- 소수점 0번째 자리에서 반올림해서
                                     -- 소수점 -1자리 표현
...
-- SYSDATE : 시스템의 현재 시간(년,월,일,시,분,)을 반환
SELECT SYSDATE FROM DUAL;

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL;
-- TIMESTAMP : 특정 시간을 나타내거나 기록하기 위한 문자열
-- 2024-02-29 11:21:05.680 +0900 (UTC에서 +9시간 나타냄, 한국 표준시)
...
-- 0 : 숫자 한칸을 의미, 여러개 작성 시 오른쪽 정렬 + 빈칸이 있으면 0 추가
...
-- HH : 시간 / HH24 : 24시간 표시법
-- MI : 분 / SS : 초
-- DAY : 요일(전체) / DY : 요일(요일명만 표시 EX:목,금,월)


SELECT SYSDATE FROM DUAL; -- 2024-02-29 12:20:56.000

-- 2024-02-29 12:20:56 목요일
...
-- NVL(컬럼명, 컬럼값 NULL일 때 바꿀값) : NULL인 컬럼값을 다른값으로 변경
...
SELECT EMP_NAME, JOB_CODE, SALARY,
	...
	DECODE(JOB_CODE, 'J7', SALARY * 1.2,
									 'J6', SALARY * 1.15,
									 'J5', SALARY * 1.1,
									 SALARY * 1.05	) "인상된 급여" -- 공백이 있어서 ""를 쓴다
FROM EMPLOYEE;
...
-- 하나 이상의 행을 그룹으로 묶어 연산하여 총합, 평균 등의 하나의 경과 행으로 반환하는 함수
...
-- 급여 최소값, 가장빠른 입사일, 알파벳순서로 가장빠른 이메일을 조회
