 ...
 * 		--> VARVHAR2(10) 컴럼에 'ABC' 3BYTE인 문자열만 저장하면 나머지 7BYTE 반환함
 * 
 * DATE : 날짜 타입
 * BLOB : 대용량 이진 데이터 (4GB)
 * CLOB : 대용량 문자 데이터 (4GB)
...
-- 문자인코딩 UTF-8 : 영어, 숫자, 1BYTE, 한글, 특수문자 등은 3BYTE 취금
...
DESC USER_CONSTRAINTS; -- SQLPLUS에서만 됨, DBeaver 에서는 안됨
SELECT * FROM USER_CONSTRAINTS;

-- USER_CONS_COLUMNS : 제약조건이 걸려있는 컬럼을 확인하는 딕셔너리 뷰
DESC USER_CONS_COLUMNS; -- 이것도 여기서 안됨
...
-- NULL을 ("KH_LHG"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- 한 개 커럶에 설정할 수 있고, 여러개의 컬럼을 묶어서 설정할 수 있음
...
INSERT INTO USER_USED_PK2
VALUES(1, 'USER01', 'PASS02', '심사임당', '여', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_LHG.PK_USERNO_USERID)에 위배됩니다
-- 회원번호와 아이디 둘다 중복되었을때만 제약조건 위배 에러 발생

INSERT INTO USER_USED_PK2
VALUES(NULL, 'USER01', 'PASS02', '심사임당', '여', '010-1234-5678', 'hong123@kh.or.kr');
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공한느 값만 사용할 수 있음
...
-- ORA-02291: 무결성 제약 조건(KH_LHG.GRADE_CODE_FK)에 위배되었습니다- 부코 키가 없습니다
...
-- 1) ON DELETE RESTRICTED(삭제 제한)로 기본 설정되어 있음
-- FOREIGN KEY로 지정된 컬럼에서 사용되고 있는
-- 제공하는 컬럼의 값은 삭제하지 못함
...
CREATE TABLE USER_TEST(
		USER_NO NUMBER CONSTRAINT PK_USER_TEST PRIMARY KEY, -- 회원번호
		USER_ID VARCHAR2(20) CONSTRAINT UK_USER_ID UNIQUE, -- 회원아이디
		USER_PWD VARCHAR2(30) CONSTRAINT NN_USER_PWD NOT NULL, -- 회원비밀번호
		PNO VARCHAR2(14) CONSTRAINT NN_PNO NOT NULL, -- 주민등록번호
		...
		CONSTRAINT CK_STATUS CHECK( STATUS IN('Y', 'N') ), -- 탈퇴여부
...
VALUES(1, 'user01', 'pass01', '880111-1234567', '남', '010-1234-5678', '인천시 미추홀구 학익동', 'N');

INSERT INTO USER_TEST
VALUES(2, 'user02', 'pass02', '880112-1234567', '여', '010-1111-2222', '인천시 미추홀구 연수동', 'N');

INSERT INTO USER_TEST
VALUES(3, 'user03', 'pass03', '880113-1234567', '남', '010-2222-3333', '인천시 미추홀구 구월동', 'Y');

INSERT INTO USER_TEST
VALUES(4, 'user04', 'pass04', '880114-1234567', '여', '010-3333-4444', '서울시 강남구', 'N');

INSERT INTO USER_TEST
VALUES(5, 'user05', 'pass05', '880115-1234567', '여', '010-4444-5555', '인천시 미추홀구 용현동', 'N');
...
--> 참조 테이블의 PK(기본키)를 FK로 사용하는 경우 참조컬럼명 생략가능
...
JOIN USER_CONS_COLUMNS C2 USING (CONSTRAINT_NAME) -- 둘다 딕셔너리 뷰
WHERE C1.TABLE_NAME = 'EMPLOYEE';
-- 이 테이블의 PK가 어떤 컬럼인지 알 수 있음
...
FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT ON DELETE SET NULL;
															/* PK 컬럼 참조(DEPT_ID) */
-- ON DELETE SET NULL : 부모가 삭제되면 자식이 NULL값으로 변경되는 삭제옵션
