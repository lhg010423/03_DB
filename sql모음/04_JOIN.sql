...
--    왼쪽               오른쪽
-- 23행인 EMPLOYEE 기준으로 만들어짐 NULL도 포함됨
...
-- 오른쪽 기준으로 만든거라 왼쪽에 값이 없어도 오른쪽 기준으로 값이 나온다
...
-- E1을 기준으로 하려면 반대인 E2.EMP_ID에 +를 붙인다
...
-- 잘못 조인하면 CROSS JOIN 결과 조회, 같은 컬럼명이 없어서 이렇게 됨
...
-- 사원이름이랑 지역명은 겹치는게 없다 사원이름이랑 부서명을 먼저 JOIN하고 난뒤에 지역명을 JOIN한다
...
WHERE E.JOB_CODE = J.JOB_CODE -- 컬럼명이 같아서 구분을 해야함
...
-- 부서가 있는 사원만 검색하는 거니까 LEFT를 안쓴다


-- 6번 문제
SELECT EMP_NAME, JOB_NAME, SALARY, 
--(SALARY * 12) + DECODE(BONUS, NULL, 0, SALARY * BONUS) 연봉
